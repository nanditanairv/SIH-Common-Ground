...
function UploadModal({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: (reference: string) => void }) {
  ...
  const [form, setForm] = useState({ name: "", description: "", phone: "", aadhar: "", email: "", location: "", latitude: "", longitude: "", urgencyRequested: false });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!form.location) { setError("Add a location or pin your current location before submitting."); return; }
    try {
      const result = await mutation.mutateAsync({ ...form, images });
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Share a challenge">
    <div className="modal-card upload-modal">
      ...
      <form onSubmit={submit} className="modal-form">
        <div className="form-grid"><label>Problem name<input required value={form.name} onChange={e => update("name", e.target.value)} placeholder="e.g. Water logging at the bus stand" /></label><label>Phone number<input required value={form.phone} onChange={e => update("phone", e.target.value)} placeholder="10-digit phone number" /></label></div>
        <label>Explain the problem<textarea required minLength={20} value={form.description} onChange={e => update("description", e.target.value)} placeholder="What is happening, who is affected and what would better look like?" rows={4} /></label>
        <div className="form-grid"><label>Aadhaar number<input required minLength={12} maxLength={16} inputMode="numeric" value={form.aadhar} onChange={e => update("aadhar", e.target.value.replace(/\D/g, ""))} placeholder="12-digit Aadhaar (stored securely)" /><small>We only retain the last four digits for your reference.</small></label><label>Email <span className="optional">optional</span><input type="email" value={form.email} onChange={e => update("email", e.target.value)} placeholder="you@example.com" /></label></div>
        <div className="form-grid"><label>Nearby location<input required value={form.location} onChange={e => update("location", e.target.value)} placeholder="Landmark, ward or village" /></label><button type="button" className="location-button" onClick={locate}><Compass size={17} /> {isLocating ? "Locating…" : "Use my location"}</button></div>
